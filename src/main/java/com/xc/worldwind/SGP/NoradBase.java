...
public abstract class NoradBase 
{
	 // Orbital parameter variables which need only be calculated one
	   ...
	   protected EciTime FinalPosition(double  incl, double omega,  double     e,   
	                                   ...
	                                   double    xn, double tsince) throws Exception
	   {
	      if ((e * e) > 1.0)
	      {
	      ...
	      Calendar gmt = Orbit.EpochTime;//.AddMinutes(tsince);
	      int s = 0, ms = 0 ;
		  s = (int)((tsince-(int)tsince)*60);
		  ms = (int) ((tsince - (int)tsince)*60 - s)*1000;
	      gmt.add(Calendar.MINUTE, (int)tsince);
	      gmt.add(Calendar.SECOND, s);
	      gmt.add(Calendar.MILLISECOND, ms);
