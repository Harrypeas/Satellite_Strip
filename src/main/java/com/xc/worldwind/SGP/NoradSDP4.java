...
public class NoradSDP4 extends NoradBase {
     ...
     {
    	 super(orbit);
         double sinarg = Math.sin(Orbit.ArgPerigee);//0.73
         double cosarg = Math.cos(Orbit.ArgPerigee);//0.67
         ...
         double eq = Orbit.Eccentricity;//0.73
         ...
         double dpi_xnodce = 4.5236020 - 9.2422029E-4 * day;//-22
     ...
     private boolean DeepCalcDotTerms( double []pxndot,  double []pxnddt,  double []pxldot)
     {
        ...
        if (gp_sync)
        {
           ...
           pxndot[0] = dp_del1 * Math.sin(dp_xli - fasx2) + 
                    dp_del2 * Math.sin(2.0 * (dp_xli - fasx4)) +
                    dp_del3 * Math.sin(3.0 * (dp_xli - fasx6));
           pxnddt[0] = dp_del1 * Math.cos(dp_xli - fasx2) +
        ...
        else
        {
           ...
           pxndot[0] = dp_d2201 * Math.sin(x2omi + dp_xli - g22) + 
           ...
           pxnddt[0] = dp_d2201 * Math.cos(x2omi + dp_xli - g22) +
        ...
        pxldot[0] = dp_xni + dp_xfact;
        pxnddt[0] = pxnddt[0] * pxldot[0];
                                     ...
                                     double []pxldot, double delta)
     {
        ...
        dp_xli = dp_xli + pxldot[0] * delta + pxndot[0] * dp_step2;
        dp_xni = dp_xni + pxndot[0] * delta + pxnddt[0] * dp_step2;
  ...
  // ///////////////////////////////////////////////////////////////////////////
                               ...
                               double []tsince_1)
     {
        ...
        xmdf[0]   = xmdf[0]   + dp_ssl * tsince_1[0];
        omgadf[0] = omgadf[0] + dp_ssg * tsince_1[0];
        xnode[0]  = xnode[0]  + dp_ssh * tsince_1[0];
        emm[0]    = Orbit.Eccentricity + dp_sse * tsince_1[0];
        xincc[0]  = Orbit.Inclination  + dp_ssi * tsince_1[0];

        if (xincc[0] < 0.0)
        {
           xincc[0]  = -xincc[0];
           xnode[0]  = xnode[0]  + Globals.Pi;
           omgadf[0] = omgadf[0] - Globals.Pi;
        }
        ...
        if (gp_reso) 
        {
           ...
           {
              if ((dp_atime == 0.0)                     ||
                 ((tsince_1[0] >= 0.0) && (dp_atime <  0.0)) ||
                 ((tsince_1[0] <  0.0) && (dp_atime >= 0.0)))
              {
                 delt =  (tsince_1[0] < 0) ? dp_stepn : dp_stepp;
              ...
              {
                 if (Math.abs(tsince_1[0]) < Math.abs(dp_atime))
                 {
                    delt = dp_stepp;
           ...
           while (Math.abs(tsince_1[0] - dp_atime) >= dp_stepp)
           {
        	   DeepCalcIntegrator(xndot, xnddt, xldot, delt);
           }

           ft = tsince_1[0] - dp_atime;

           DeepCalcDotTerms(xndot, xnddt, xldot);

           xnn[0] = dp_xni + xndot[0] * ft + xnddt[0] * ft * ft * 0.5;

           double xl   = dp_xli + xldot[0] * ft + xndot[0] * ft * ft * 0.5;
           double temp = -xnode[0] + dp_thgr + tsince_1[0] * thdt;

           xmdf[0] = xl - omgadf[0] + temp;
                                 ...
                                 double []xmam,   double tsince)
     {
        ...
        double sinis = Math.sin(xincc[0]);
        double cosis = Math.cos(xincc[0]);
        ...
        else
        {
           ...
           double sinok = Math.sin(xnode[0]);
           double cosok = Math.cos(xnode[0]);
           ...
           double xls = xmam[0] + omgadf[0] + cosis * xnode[0];
           double dls = pl + pgh - pinc * xnode[0] * sinis;

           xls    = xls + dls;
           xnode[0]  = Globals.AcTan(alfdp, betdp);
           xmam[0]   = xmam[0] + pl;
           omgadf[0] = xls - xmam[0] - Math.cos(xincc[0]) * xnode[0];
        }
	...
	public EciTime GetPosition(double tsince) {
		// TODO 自动生成的方法存根
		 // Update for secular gravity and atmospheric drag （更新secular gravity和大气阻力）
		double xmdf_1=Orbit.MeanAnomaly + m_xmdot * tsince;
        double []xmdf   = {xmdf_1};
        ...
        double []em     = {0.0};
        double []xinc   = {0.0};
        double []tsince_1={tsince};
        
        
        DeepSecular( xmdf,  omgadf,  xnode,  em,  xinc,  xn,  tsince_1);

        double a    = Math.pow(Globals.Xke / xn[0], 2.0 / 3.0) * Globals.Sqr(tempa);
        double []e    = {em[0] - tempe};
        double []xmam = {xmdf[0] + m_xnodp * templ};
        double tsince_2=tsince_1[0];
        
        DeepPeriodics( e,  xinc,  omgadf,  xnode,  xmam, tsince_2);

        double xl = xmam[0] + omgadf[0] + xnode[0];

        xn[0] = Globals.Xke / Math.pow(a, 1.5);
        EciTime eciTime=null;
        try {
        	eciTime= FinalPosition(xinc[0], omgadf[0], e[0], a, xl, xnode[0], xn[0], tsince_2);
		} catch (Exception e1) {
			// TODO 自动生成的 catch 块
			e1.printStackTrace();
		}
